...
class IDL_Demo_Content {
	...
	public function register_hooks(): void {
		add_action( 'admin_post_idl_install_demo', [ $this, 'handle_install' ] );
		add_action( 'admin_post_idl_remove_demo', [ $this, 'handle_remove' ] );
	}
	...
	private function create_categories(): array {
		...
		foreach ( $tree as $slug => $node ) {
			...
			$result = wp_insert_term( $node['name'], 'idl_category', [
				'slug'   => $slug,
				'parent' => $parent_id,
			] );
	...
	private function category_tree( bool $sr ): array {
		return [
			'municipal-assembly'      => [ 'name' => $sr ? 'Скупштина општине'       : 'Municipal Assembly' ],
			'term-2025-2029'          => [ 'name' => $sr ? 'Сазив 2025-2029'         : 'Term 2025-2029',          'parent' => 'municipal-assembly' ],
			'session-i'               => [ 'name' => $sr ? 'I седница'               : 'Session I',               'parent' => 'term-2025-2029' ],
			'session-ii'              => [ 'name' => $sr ? 'II седница'              : 'Session II',              'parent' => 'term-2025-2029' ],
			'term-2021-2025'          => [ 'name' => $sr ? 'Сазив 2021-2025'         : 'Term 2021-2025',          'parent' => 'municipal-assembly' ],
			'municipal-council'       => [ 'name' => $sr ? 'Општинско веће'          : 'Municipal Council' ],
			'decisions'               => [ 'name' => $sr ? 'Одлуке'                  : 'Decisions',               'parent' => 'municipal-council' ],
			'resolutions'             => [ 'name' => $sr ? 'Решења'                  : 'Resolutions',             'parent' => 'municipal-council' ],
			'public-procurement'      => [ 'name' => $sr ? 'Јавне набавке'           : 'Public Procurement' ],
			'open-procedures'         => [ 'name' => $sr ? 'Отворени поступци'       : 'Open Procedures',         'parent' => 'public-procurement' ],
			'negotiated-procedures'   => [ 'name' => $sr ? 'Преговарачки поступци'   : 'Negotiated Procedures',   'parent' => 'public-procurement' ],
			'urban-planning'          => [ 'name' => $sr ? 'Урбанизам'               : 'Urban Planning' ],
			'finance'                 => [ 'name' => $sr ? 'Финансије'               : 'Finance' ],
			'budget'                  => [ 'name' => $sr ? 'Буџет'                   : 'Budget',                  'parent' => 'finance' ],
			'final-account'           => [ 'name' => $sr ? 'Завршни рачун'           : 'Final Account',           'parent' => 'finance' ],
		];
	}
